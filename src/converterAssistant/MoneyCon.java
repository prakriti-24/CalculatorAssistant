...
import javax.swing.SwingUtilities;
    ...
    public class MoneyCon extends JFrame {
    ...
    private static final String[] TK_UNITS = {"Tk","Euro","dollar","rupi","riyal","Prupee","Yen"};

    public MoneyCon() {

        this.setLayout(new GridLayout(5, 2, 50, 50));
        inputUnitComboBox = new JComboBox(TK_UNITS);
        outputUnitComboBox = new JComboBox(TK_UNITS);
    ...
    private double convert(double value, String inputUnit, String outputUnit) {
        ...
        if (inputUnit.equals("Euro")) {
            value = value/0.0099;
        } else if (inputUnit.equals("dollar")) {
            value = value/0.012;
        }
        
            else if (inputUnit.equals("rupi")) {
            value = value/0.86;    
        }
           else if(inputUnit.equals("riyal")){
                value = value/0.035;
            }
            else if(inputUnit.equals("Yen")){
                value=value/1.31;
            }
            else if(inputUnit.equals("Prupee")){
                value=value/2.64;
            }
                

        // Convert to the target output unit
        if (outputUnit.equals("Euro")) {
            value = value*0.0099;
        } else if (outputUnit.equals("dollar")) {
            value = value*0.012;
        }
            else if (outputUnit.equals("rupi")) {
            value = value*0.86;    
        }
            else if(outputUnit.equals("riyal")){
                value = value*0.035;
            }
  
         else if (outputUnit.equals("Yen")) {
            value = value/1.31;
        }
            else if (outputUnit.equals("Prupee")) {
            value = value/2.64;    
        }
